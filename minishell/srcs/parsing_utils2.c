...
/*   Updated: 2022/10/17 17:02:43 by dpaulino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *buffer, char **envp)
{
	...
	while (buf_s->exe && buf_s->exe[buf_s->i])
	{
		if (buf_s->exe[buf_s->i][0] == ' ')
			buf_s->exe[buf_s->i]++;
		buf_s->args = ft_calloc(200, sizeof(char *));
		parse_quotes(buf_s->args, buf_s->exe[buf_s->i]);
		if (buf_s->exe[buf_s->i] && find_char(buf_s->exe[buf_s->i], "$"))
